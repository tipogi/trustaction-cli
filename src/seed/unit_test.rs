...
mod tests {
    ...
    fn test_words_master_key() {
        let mnemonic = _read_mnemonic_phrase();
        let master_key = _create_master_key_from_bdk_library(mnemonic);
    ...
    fn test_hex_master_key() {
        let hex_mnemonic = _mnemonic_hex();
        let master_key = _create_master_key_from_bitcoin_library(hex_mnemonic);
    ...
    fn test_extended_keys() {
        ...
        let extended_keys = _generate_keys(DERIVATION_PATH);
    ...
    fn test_extended_key_descriptor() {
        ...
        let private_descriptor = _create_descriptor(&extended_keys, DERIVATION_PATH, KeyType::Private);
        let public_descriptor = _create_descriptor(&extended_keys, DERIVATION_PATH, KeyType::Public);
    ...
    fn create_multiple_addresses() {
        ...
        for index in 0..=5 {
            ...
            _create_address_and_its_keys(&address_derivation_path, index);
        }
    }
}